...
class Seller extends Model
{
    public static function SellerPost(){
            ...
            $buyer = DB::select('select id,name from lgtks_users where is_seller=1');
      ...
      // inserting add routes data to lgkts_posts table
        if(empty($nextdata)){
            ...
            foreach($nextdata as $discData){
              ...
              $prt_data=array('lgtks_post_id'=>$Private_post_id,'user_id'=>$userid);
...
or lp.veh_type=sh.vehicle_type and lu.is_seller=0'));
  ...
  public static function SellerPostMasterTo($user_id){
   ...
   $SellerPostMasterTo = DB::select(DB::raw("select lu.is_seller as is_seller1,lp.is_seller as is_seller,lu.name as name, title, lp.private_public_flag as private_public_flag,
...
(select count(lp.id) from lgtks_posts lp join lgtks_users lu where enquiry_status=1 and lp.is_seller=0 and lu.is_seller=1 and lu.id=1)as enquiry,
valid_from,valid_to from lgtks_posts lp join lgtks_users lu on lp.user_id=lu.id where lu.is_seller=0 and lp.is_seller=1 and lu.id=:user_id"), array(
'user_id'=>$user_id,));

     $to = DB::select(DB::raw("select count(id) as outgoing_posts from lgtks_posts where user_id=:user_id"), array('user_id'=>$user_id,));
     $from = DB::select(DB::raw("select count(lp.id) as incoming_posts from lgtks_posts lp join private_posts pp on pp.lgtks_post_id=lp.id where private_public_flag=1 and lp.user_id=:user_id"),array('user_id'=>$user_id,));
     ...
     public static function SellerPostMasterFrom($user_id){
        ...
        $sellerPostmasterFrom=DB::select(DB::raw('select lu.is_seller as is_seller,lp.is_seller as is_seller,lu.name as name, title, lp.private_public_flag as private_public_flag,
datediff(valid_to,valid_from) as DiffDate,
(select sub_type_1 from lgtks_catalogues where id=(lp.catalogue_id)) as catalogue,
(select count(lp.id) from lgtks_posts lp join lgtks_users lu where enquiry_status=1 and lp.is_seller=0 and lu.is_seller=1 and lu.id=1)as enquiry,
 (select count(pi.id) from post_interactions pi where message_to=lu.id and pi.lgtks_post_id)=lp.id as Messages,
 valid_from,valid_to  from  lgtks_posts lp  join lgtks_users lu on lp.user_id=lu.id where lu.is_seller=0 and lp.user_id=:user_id'),array('user_id'=>$user_id,)); 
     ...
     public static function SellerPostMasterAll($user_id){
          ...
          $sellerPostmasterAll=DB::select(DB::raw('select lu.is_seller as is_seller,lp.is_seller as is_seller,lu.name as name, title, lp.private_public_flag as private_public_flag,
datediff(valid_to,valid_from) as DiffDate,(select contract_name from contract_types where id=(lp.contract_type)) as contract,
(select sub_type_1 from lgtks_catalogues where id=(lp.catalogue_id)) as catalogue,
(select count(pi.id) from post_interactions pi where message_to=lu.id and pi.lgtks_post_id)=lp.id as Messages,
(select count(lp.id) from lgtks_posts lp join lgtks_users lu where enquiry_status=1 and lp.is_seller=0 and lu.is_seller=1 and lu.id=1)as enquiry,
valid_from,valid_to from lgtks_posts lp  join lgtks_users lu on lp.user_id=lp.user_id 
 where lu.is_seller=1 or lp.is_seller=0 and lp.user_id=:user_id'),array('user_id'=>$user_id,)); 
...
public static function PostMGData($id){
    ...
    public static function fetchSellerEnquiries($user_id,$lgtks_post_id){
        $data = DB::select(DB::raw('select lp.price,lp.id,(select village from pincode_location_details where id= lp.from_loc)as from_loc,lp.is_seller,
...
where lp.private_public_flag=0 and lp.enquiry_status=1 and lp.is_seller=1 and lp.user_id=:user_id'),array('user_id'=>$user_id,));
        
        $cnt_p_t= DB::select(DB::raw("select count(pi.id) as cnt_id from post_interactions pi join lgtks_posts lp  on lgtks_post_id=lp.id  where is_seller=1 and post_interaction_type=2 and lp.id=:lgtks_post_id"),array('lgtks_post_id'=>$lgtks_post_id,));
