...
class EmailSender {
    ...
    public function sendMailToSeller($input) {
        ...
        return sendEmail($config, ['description' => 'http://www.logistiks.com/user_activation?key='.$token.'&registered_id='.$registered_id, 'email' => $input[0]->email_id , 'name' => $input[0]->first_name,'token'=>$token,'registered_id'=>$registered_id ]);
    ...
    public function sendMailToUser($input) {
        ...
        return sendEmail($config, ['description' => 'http://www.logistiks.com/new_user_acftivation?key='.$token.'&user_id='.$user_id, 'email' => $input['email_id'] , 'name' => $input['user_name'],'token'=>$token,'user_id'=>$user_id ]);
